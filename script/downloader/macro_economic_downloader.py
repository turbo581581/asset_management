...
def download_us_ecomonic(save_path, start, end):
    ...
    capital_inflow.to_csv(os.path.join(save_path, "1_capital_inflow", "capital_inflow.csv"), header=['capital_inflow'])
    ...
    m0.to_csv(os.path.join(save_path, "2_monetary_and_fiscal_policy", "m1.csv"), header=['m1'])
    # government budget
    ...
    sp500_filter = sp500["Close"]
    sp500.to_csv(os.path.join(save_path, '4_market_status', "sp500_full.csv"), index=True)
    # bonds
